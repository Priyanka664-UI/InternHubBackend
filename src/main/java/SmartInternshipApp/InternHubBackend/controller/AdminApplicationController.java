...
public class AdminApplicationController {
    ...
    public ResponseEntity<List<InternshipApplication>> getAllApplications(
        ...
        if (userType != null && userType == 2 && companyId != null) {
            applications = applications.stream()
                .filter(app -> app.getInternship() != null && companyId.equals(app.getInternship().getCompanyId()))
    ...
    public ResponseEntity<List<InternshipApplication>> getIndividualApplications(
            @RequestHeader(value = "X-User-Type", required = false) Integer userType,
            @RequestHeader(value = "X-Company-Id", required = false) Long companyId) {
        
        List<InternshipApplication> applications = applicationRepository.findByApplicationType(InternshipApplication.ApplicationType.INDIVIDUAL);
    ...
    @GetMapping("/group")
    public ResponseEntity<List<InternshipApplication>> getGroupApplications(
            @RequestHeader(value = "X-User-Type", required = false) Integer userType,
            @RequestHeader(value = "X-Company-Id", required = false) Long companyId) {
        
        List<InternshipApplication> applications = applicationRepository.findByApplicationType(InternshipApplication.ApplicationType.GROUP);
